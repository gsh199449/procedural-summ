...
class MetricCompute:
    ...
    def compute_metrics(self, eval_preds):
        ...
        input_content = [item for sublist in dataset['text'] for item in sublist]
        ...
        rouge_result = self.rouge_metric.compute(predictions=rouge_decoded_preds, references=rouge_decoded_labels, use_stemmer=True)
        # Extract a few results from ROUGE
        result.update({key: value.mid.fmeasure * 100 for key, value in rouge_result.items()})

        # evaluate bleu
        bleu_decoded_preds, bleu_decoded_labels = self.postprocess_text('sacrebleu', decoded_preds, decoded_labels)
        bleu_decoded_preds, bleu_decoded_labels = list(zip(*[(p, l) for p, l in zip(bleu_decoded_preds, bleu_decoded_labels) if l[0].strip() != '']))
        try:
            bleu_result = self.bleu_metric.compute(predictions=bleu_decoded_preds, references=bleu_decoded_labels)
        ...
        bleu_decoded_preds, bleu_decoded_labels = self.postprocess_text('yiping_bleu', decoded_preds, decoded_labels)
        ...
        yiping_bleu_result = self.yiping_bleu_metric.compute(predictions=bleu_decoded_preds, references=bleu_decoded_labels,
